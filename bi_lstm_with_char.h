...
void networkForward(Sequence & s, Eigen::MatrixXd & loss){
...
void networkBackward(Sequence & s){
    int sequenceLen = s.charEmb.size();
...
void paramGradCheck(Sequence s,
...
void inputGradCheck(Sequence & s){
    ...
    for (int i = 0; i < num_checks; ++i) {
        int randRow = 0 + (rand() % (int)(s.wordEmb.rows()));
        int randCol = 0 + (rand() % (int)(s.wordEmb.cols()));

        double originalVal = s.wordEmb(randRow, randCol);

        Eigen::MatrixXd loss0;

        s.wordEmb(randRow, randCol) = originalVal - delta;
        ...
        s.wordEmb(randRow, randCol) = originalVal;
...
void networkGradientCheck(Sequence & s){
...
void biLSTMCharRun(std::vector<Sequence>& training,
                   std::vector<Sequence>& eval,
                   int epoch
                   ) {
    //
